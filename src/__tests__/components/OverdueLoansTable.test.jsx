...
describe('OverdueLoansTable component (HU-05)', () => {
  ...
  test('TC-HU05-01 — renders table with expected column headers', async () => {
    ...
    expect(screen.getByText(/Lector Responsable/i)).toBeInTheDocument();
  ...
  // ─── TC-HU05-01: days overdue shown with suffix ──────────────
  test('TC-HU05-01 — displays days overdue with "días" suffix', async () => {
    mockGetOverdue = vi.fn().mockResolvedValue([OVERDUE_LOAN_01]);

    render(<OverdueLoansTable />);

    await screen.findByText('La Odisea');
    // The days overdue column shows "X días" or "X día"
  ...
  // ─── TC-HU05-01: state badge shows Vencido ───────────────────
  test('TC-HU05-01 — displays Vencido state badge for overdue loans', async () => {
    mockGetOverdue = vi.fn().mockResolvedValue([OVERDUE_LOAN_01]);
