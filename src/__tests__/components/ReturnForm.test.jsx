...
describe('ReturnForm — interactions (HU-03)', () => {
  ...
  test('displays success message after successful return', () => {
    ...
    expect(screen.getByText(/No hay multa por esta devolución/i)).toBeInTheDocument();
  });
  ...
  test('shows search error when neither id_book nor id_reader provided', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Debe proporcionar ID del libro o ID del lector/i)).toBeInTheDocument();
    });
