...
describe('LoanSearch component (HU-01)', () => {
  ...
  // ─── Validación: submit con input vacío muestra error ──────────
  test('shows validation error when submitting with empty input', async () => {
    render(<LoanSearch />);
    const submitBtn = screen.getByRole('button', { name: /buscar/i });

    await userEvent.click(submitBtn);

    expect(screen.getByText(/Ingresa el nombre del libro/i)).toBeInTheDocument();
